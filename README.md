...
- [x] 전체 투표 목록(제목, 생성자, 만료 날짜 및 시간, 진행 중 여부)이 보여야 합니다.
  └ [ ] 데이터로딩 실패에 관한 에러처리[mongoose]
  └ [ ] 데이터로딩 실패에 관한 에러처리[mongoDB]
  └ [ ] 데  이터가 아무것도 없을 경우에 관한 렌더링 처리
- [x] 투표를 클릭할 경우, `/votings/:id`로 이동합니다.
  └ [ ] 상세페이지에 진입했을 때 데이터를 가지고 오지 못할때 에러처리
...
- [x] 만료된 투표의 경우, 투표 결과를 표기해주어야 합니다. (가장 많은 선택을 받은 사항 표기)
  └ [ ] 투표가 만료된 상황에서 사용자가 의도적으로 투표에 참여할 경우, 반영되지 않도록 처리
- [x] 진행 중인 투표의 경우, 누구든지 투표에 참여할 수 있어야 합니다.
- [x] 중복 투표는 불가능합니다.
  └ [ ] 중복 투표 진행시 사용자에게 에러페이지 렌더링
...
- [x] 투표 생성자가 "삭제"할 경우, 더 이상 투표는 참가할 수 없습니다.
  └ [ ] Restful하게 수정(클라이언트에서 요청처리)
  └ [ ] 삭제 실패에 관한 에러처리
  └ [ ] 투표를 삭제할 경우, 투표를 생성한 유저의 배열에서 삭제 필요
...
- [x] 투표 생성 성공 페이지는 필요에 따라 다른 방식으로 대체하여도 괜찮습니다.
  └ [ ] 투표 참여 이후 성공에 관한 메세지 (클라이언트단에서 처리)
...
- [x] 투표 생성 실패 페이지는 필요에 따라 다른 방식으로 대체하여도 괜찮습니다.
  └ [ ] 투표 참여 이후 실패에 관한 메세지 (클라이언트단에서 처리)
